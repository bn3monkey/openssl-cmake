...
#define LOG_D(...) __android_log_print(ANDROID_LOG_DEBUG, "openssl_cmake", "%s", __VA_ARGS__);
...
int main()
{
    ...
    if (!crypto_ok || !ssl_ok)
    {
        ...
        LOG_D("OpenSSL TLS self-check FAILED\n");
        return 1;
    }

    LOG_D("OpenSSL TLS self-check PASSED\n");
    return 0;
}

extern "C" JNIEXPORT void JNICALL
Java_com_bn3monkey_openssl_1cmake_TestClass_main(
        JNIEnv* env,
        jobject /* this */) {
    main();
}